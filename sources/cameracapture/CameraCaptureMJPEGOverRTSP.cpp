#include "CameraCaptureMJPEGOverRTSP.h"

vms::cameracapture::CameraCaptureMJPEGOverRTSP::CameraCaptureMJPEGOverRTSP(const std::string& video_stream_address, const std::string& login, const std::string& password) {
    m_video_stream_address = video_stream_address;
    m_login = login;
    m_password = password;
    try {
        m_capture = new cv::VideoCapture("rtsp://" + login + ":" + password + "@" + video_stream_address, CV_CAP_FFMPEG);
        m_capture->set(cv::CAP_PROP_FOURCC, CV_FOURCC('M', 'J', 'P', 'G'));
        cv::Mat frame;
...
std::tuple<std::shared_ptr<std::vector<unsigned char>>, cv::Mat> vms::cameracapture::CameraCaptureMJPEGOverRTSP::fetchNextFrame() {
